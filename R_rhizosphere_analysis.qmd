...
file_paths_total <- list.files("/Volumes/OWC_Gemini/hazelnut_rhizosphere_November_25_2025/rhizosphere_2025/krona_input_2024_09_25/", pattern = "(phylum|phylum_bac|phylum_arc|phylum_euk|class|class_bac|class_arc|class_euk|genus|genus_bac|genus_arc|genus_euk|species|species_bac|species_arc|species_euk)[.]txt$", recursive = TRUE, full.names = TRUE)
...
# Function to transform data in list of data frames of interest to proper format (relative abundance)
...
# Function to transform data in list of data frames of interest to proper format (counts data)
transform_df_list_obs <- function(df_list, source_col, taxa_col, num_col) {
  result <- list()
  list_names <- names(df_list)
  for (i in list_names) {
    current_df <- df_list[[i]]
    result[[i]] <- current_df |> group_by({{source_col}}, {{taxa_col}}) |>
      ...
      ungroup() |>
      distinct() |>
      select({{taxa_col}}, {{num_col}}, {{source_col}}) |>
...
# Transfrom data frames in list (observed)
refined_mmseqs_df_list_obs <- transform_df_list_obs(mmseqs_df_list, source_file, V2, V1)
...
# Get phlyoseq objects form list of data frames (observed) 
physeq_object_list_mmseqs_obs <- process_df_to_phyloseq(refined_mmseqs_df_list_obs)

# Convert all phyloseq objects in list to integers (observed)
phyloseq_list_int <- lapply(physeq_object_list_mmseqs_obs, function(pl) {
  otu_table(pl) <- round(otu_table(pl))
  return(pl)
})

## Upload bracken data (trimmed) (phylum, genus, species)
combined_phylum_bracken_trim <- read.delim("/Volumes/OWC_Gemini/bracken_2026/kraken2_output_confidence_0.51/trimmomatic/combined.phylum.bracken", header = TRUE)
combined_genus_bracken_trim <- read.delim("/Volumes/OWC_Gemini/bracken_2026/kraken2_output_confidence_0.51/trimmomatic/combined.genus.bracken", header = TRUE)
combined_species_bracken_trim <- read.delim("/Volumes/OWC_Gemini/bracken_2026/kraken2_output_confidence_0.51/trimmomatic/combined.species.bracken", header = TRUE)
...
## phyloseq object or obs
physeq_object_list_bracken_num <- process_df_to_phyloseq(bracken_list_num)
...
get_alpha_bar <- function(physeq_object_list, alpha_diversity_measure) {
  ...
  for (i in list_names) {
    ...
    current_alpha_final <- current_alpha +
      ...
      geom_jitter(width = 0.08, aes(group = NULL)) +
      labs(x = "",
           y = paste0(alpha_diversity_measure, " Species Richness")) +
      scale_color_manual(values = colours) +
    ...
    } else if(alpha_diversity_measure == "Observed") {
      current_alpha_final <- current_alpha_final +
...
get_kruskal_wallis <- function(physeq_object_list, measure_type) {
  ...
  for (i in list_names) {
    current_physeq_obj <- physeq_object_list[[i]]
    current_alpha <- estimate_richness(current_physeq_obj, measures = c(measure_type))
...
## Get observed mmseqs
alpha_bracken_obs <- get_alpha_bar(physeq_object_list_bracken_num, "Observed")
alpha_mmseqs_obs <- get_alpha_bar(phyloseq_list_int, "Observed")
...
kruskal_test_mmseqs_obs <- get_kruskal_wallis(phyloseq_list_int, "Observed")
...
# Save species level diversity plots as .pdf and .svg
ggsave("/Users/johnrobinson/Desktop/gd_March_29_2026/observed_species_mmseqs.pdf", plot = alpha_mmseqs_obs[[13]], width = 6, height = 7, units = "in")

ggsave("/Users/johnrobinson/Desktop/gd_March_29_2026/observed_species_mmseqs.svg", plot = alpha_mmseqs_obs[[13]], width = 6, height = 7, units = "in")
```
...
file_paths_subset_total <- list.files("/Volumes/OWC_Gemini/hazelnut_rhizosphere_November_25_2025/rhizosphere_2025/subset_mmseqs2_output_2024_10_05/krona_input/", pattern = "(genus|species)[.]txt$", recursive = TRUE, full.names = TRUE)
...
refined_mmseqs_df_subset_list_obs <- transform_df_list_obs(mmseqs_subset_df_list, source_file, V2, V1)

# Get phyloseq objects from list of data frames 
physeq_object_subset_list_mmseqs <- process_df_to_phyloseq(refined_mmseqs_df_subset_list)
physeq_object_subset_list_mmseqs_obs <- process_df_to_phyloseq(refined_mmseqs_df_subset_list_obs)

# Convert all phyloseq objects in list to integers (observed)
phyloseq_list_int_subset <- lapply(physeq_object_subset_list_mmseqs_obs, function(pl) {
  otu_table(pl) <- round(otu_table(pl))
  return(pl)
})

# Sample order (order samples for alpha diversity)
desired_order <- c("B2", "B3", "B4", "B5", "R1P1", "R1P5", "R1P9", "R1P14", "R1P18", "R3P3", "R3P5", "R3P11", "R3P15", "R3P20", "R3P9_neg", "R3P10_neg", "R3P12_neg", "R3P14_neg", "R3P18_neg")
...
alpha_mmseqs_subset_obs <- get_alpha_bar(phyloseq_list_int_subset, "Observed")
...
kruskal_test_mmseqs_subset_si <- get_kruskal_wallis(physeq_object_subset_list_mmseqs, "Simpson")

kruskal_test_mmseqs_subset_obs <- get_kruskal_wallis(phyloseq_list_int_subset, "Observed")
...
# Execute as .pdf and .svg
ggsave("/Users/johnrobinson/Desktop/gd_March_29_2026/observed_species_mmseqs_subset.pdf", plot = alpha_mmseqs_subset_obs[[2]], width = 6, height = 7, units = "in")

ggsave("/Users/johnrobinson/Desktop/gd_March_29_2026/observed_species_mmseqs_subset.svg", plot = alpha_mmseqs_subset_obs[[2]], width = 6, height = 7, units = "in")
```
